...
**Booking** -- the main resource the API and frontend work with.

| Field | Type | Rules |
|---|---|---|
| `id` | string | unique |
| `desk` | string | 3 to 100 characters, trimmed |
| `floor` | string | 5 to 200 characters, trimmed, e.g. "Floor 2, North Wing" |
| `date` | string | a valid ISO 8601 date |
| `active` | boolean | defaults to true |

**User** -- added in the end-to-end API session.
...
A User has many Bookings. A Booking belongs to one User through `createdById`.

**Desk** -- the read-only listing used in the infrastructure and performance sessions.

| Field | Type |
|---|---|
| `id` | number |
| `name` | string, e.g. "Window Desk A" |
| `floor` | number |
| `booked` | boolean |
...
**HTTP status codes.** `200` read or update succeeded, `201` created, `204` deleted with no
body, `400` the request was malformed or failed validation, `401` not signed in, `403` signed
in but not allowed, `404` not found, `500` something broke on our side.
